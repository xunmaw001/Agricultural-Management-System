        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '农事安排标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.nongshianpaiName" placeholder="农事安排标题" clearable></el-input>
                     </el-form-item>
                                         
                     <el-form-item :label="contents.inputTitle == 1 ? '农事安排类型' : ''">
                        <el-select v-model="searchForm.nongshianpaiTypes" placeholder="请选择农事安排类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in nongshianpaiTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('nongshianpai','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/nongshiguanli/upload/nongshianpaiMuBan.xls"
                        >批量导入农事安排数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('nongshianpai','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="nongshianpaiUploadSuccess"
                                :on-error="nongshianpaiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('nongshianpai','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入农事安排数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('nongshianpai','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "nongshianpai.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="nongshianpaiUuidNumber"
                                   header-align="center"
                                   label="农事安排编号">
                        <template slot-scope="scope">
                            {{scope.row.nongshianpaiUuidNumber}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="nongshianpaiName"
                                   header-align="center"
                                   label="农事安排标题">
                        <template slot-scope="scope">
                            {{scope.row.nongshianpaiName}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="nongshianpaiFile"
                                     header-align="center"
                                     width="200"
                                     label="农事安排附件">
                        <template slot-scope="scope">
                            <div v-if="scope.row.nongshianpaiFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.nongshianpaiFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="nongshianpaiAddress"
                                   header-align="center"
                                   label="安排地点">
                        <template slot-scope="scope">
                            {{scope.row.nongshianpaiAddress}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="nongshianpaiTime"
                                   header-align="center"
                                   label="要执行时间">
                        <template slot-scope="scope">
                            {{scope.row.nongshianpaiTime}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="nongshianpaiTypes"
                                      header-align="center"
                                      label="农事安排类型">
                        <template slot-scope="scope">
                            {{scope.row.nongshianpaiValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('nongshianpai','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('nongshianpai','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('nongshianpai','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              nongshianpaiTypesSelectSearch : [],
            form:{
                id : null,
                yuangongId : null,
                nongshianpaiUuidNumber : null,
                nongshianpaiName : null,
                nongshianpaiFile : null,
                nongshianpaiAddress : null,
                insertTime : null,
                nongshianpaiTime : null,
                nongshianpaiTypes : null,
                nongshianpaiContent : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '农事安排编号': "nongshianpaiUuidNumber",
                     '农事安排标题': "nongshianpaiName",
                     '农事安排附件': "nongshianpaiFile",
                     '安排地点': "nongshianpaiAddress",
                     '安排时间': "insertTime",
                     '要执行时间': "nongshianpaiTime",
                     '农事安排类型': "nongshianpaiValue",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'nongshianpai',//当前表表名,
                        sumColum : 'nongshianpai_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'nongshianpai_name',//分组字符串字段
                        // types : 'nongshianpai_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "nongshianpai_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.nongshianpaiName!= '' && this.searchForm.nongshianpaiName!= undefined) {
                    params['nongshianpaiName'] = '%' + this.searchForm.nongshianpaiName + '%'
                }
                                         
                if (this.searchForm.nongshianpaiTypes!= '' && this.searchForm.nongshianpaiTypes!= undefined) {
                    params['nongshianpaiTypes'] = this.searchForm.nongshianpaiTypes
                }
                
                params['nongshianpaiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "nongshianpai/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=nongshianpai_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.nongshianpaiTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            nongshianpaiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "nongshianpai/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
